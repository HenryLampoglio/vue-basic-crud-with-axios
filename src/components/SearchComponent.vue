<template>
  ...
  <Card v-if="itemCardIF && props.isProduct" class="item-card">
    <template #title
      ><div class="title-card">{{ item.name_product }}</div></template
    >
    <template #content>
      Código: {{ item.id }} <br />
      Preço: {{ item.price }} <br />
  ...
  <Card v-else-if="itemCardIF && props.isDistributor">
    <template #title
      ><div class="title-card">{{ item.company_name }}</div></template
    >
    <template #content>
      Código: {{ item.id }} <br />
...
const showItem = () => {
  ...
  if (idValue.value.length == 36) {
    if (props.isProduct) {
      ...
      apiClient
        .get(
          `${props.path}/{distributor_id}?ditributor_id=${idValue.value}`,
          {}
        )
        .then(function (response) {
          itemCardIF.value = true;
...
const deleteItem = () => {
  apiClient
    .delete(`${props.path}/${idValue.value}`, {})
    .then(function (response) {
