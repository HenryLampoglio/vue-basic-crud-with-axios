<template>
  <BaseModal v-if="modalOpen" @click="onCloseModal()" :title="modalTitle" :open="modalOpen" :variant="modalIf">
...
const searchItem = () => {
  ...
  if (props.isDistributorForm) {
    apiClient
      .get(`distributors/{distributor_id}?ditributor_id=${idValue.value}`)
...
const databaseResponse = () => {
  if (!props.isUpdateForm) {
    if (props.isDistributorForm) {
      apiClient
        .post("distributors/", distributor.value)
        .then(function (response) {
    ...
    if (props.isDistributorForm) {
      apiClient
        .put(`distributors/${idValue.value}`, distributor.value)
        .then(function (response) {
          modalIf.value = "success";
          modalTitle.value = "SUCESSO";
          modalText.value = "Distribuidor criado com sucesso";
      ...
      apiClient
        .put(`products/${idValue.value}`, product.value)
        .then(function (response) {
          modalIf.value = "success";
          modalTitle.value = "SUCESSO";
          modalText.value = "Produto criado com sucesso";
